...
namespace Controller {
  ...
  IOReturn Xbox360ControllerClass::setProperties(OSObject *properties) {
    ...
    if (owner == NULL) return kIOReturnUnsupported;
    return owner->setProperties(properties);
  }

  // Returns the HID descriptor for this device
  IOReturn Xbox360ControllerClass::newReportDescriptor(
                                                       IOMemoryDescriptor **descriptor) const {
                                                         ...
                                                         buffer->writeBytes(0, HID_360::ReportDescriptor,
                                                                            sizeof(HID_360::ReportDescriptor));
                                                         *descriptor = buffer;
                                                         return kIOReturnSuccess;
                                                       }

  // Handles a message from the userspace IOHIDDeviceInterface122::setReport
  // function
  ...
  // Get report
  IOReturn Xbox360ControllerClass::getReport(IOMemoryDescriptor *report,
                                             IOHIDReportType reportType,
                                             IOOptionBits options) {
  ...
  IOReturn Xbox360ControllerClass::handleReport(IOMemoryDescriptor *descriptor,
                                                IOHIDReportType reportType,
                                                IOOptionBits options) {
    if (descriptor->getLength() >= sizeof(XBOX360_IN_REPORT)) {
      IOBufferMemoryDescriptor *desc =
      OSDynamicCast(IOBufferMemoryDescriptor, descriptor);
      if (desc != NULL) {
        XBOX360_IN_REPORT *report = (XBOX360_IN_REPORT *)desc->getBytesNoCopy();
        if ((report->header.command == inReport) &&
            (report->header.size == sizeof(XBOX360_IN_REPORT))) {
          GetOwner(this)->fiddleReport(report->left, report->right);
          if (!(GetOwner(this)->noMapping)) remapButtons(report);
          if (GetOwner(this)->swapSticks) remapAxes(report);
        }
      }
    }
  ...
  // Returns the string for the specified index from the USB device's string list,
  // with an optional default
  OSString *Xbox360ControllerClass::getDeviceString(UInt8 index,
                                                    const char *def) const {
    if (const StandardUSB::StringDescriptor *descriptor =
        GetOwnerProvider(this)->getStringDescriptor(index)) {
      return OSString::withCString((char *)descriptor->bString,
                                   descriptor->bLength);
    } else {
...
#pragma warning TODO(fill out)
    return OSString::withCString("TODO(bojanin");
    // return
    // getDeviceString(GetOwnerProvider(this)->getManufacturerStringIndex());
  }
  ...
  OSString *Xbox360ControllerClass::newSerialNumberString() const {
    // TODO(bojanin): fill this out
    return OSString::withCString("asdf");
    // return
    // getDeviceString(GetOwnerProvider(this)->GetSerialNumberStringIndex());
  }
  ...
  OSNumber *Xbox360ControllerClass::newVendorIDNumber() const {
    // TODO(bojanin): fill this out
    return OSNumber::withNumber(1, 8);
    // TODO(bojanin): Figure this shit out
  ...
  OSNumber *Xbox360ControllerClass::newLocationIDNumber() const {
    ...
    if (device) {
      if ((number = OSDynamicCast(OSNumber, device->getProperty("locationID")))) {
        ...
        // Make up an address
        if ((number =
             OSDynamicCast(OSNumber, device->getProperty("USB Address"))))
  ...
  void Xbox360ControllerClass::remapButtons(void *buffer) {
    ...
    new_buttons |= ((report360->buttons & 16) == 16)
    << GetOwner(this)->mapping[4];
    new_buttons |= ((report360->buttons & 32) == 32)
    << GetOwner(this)->mapping[5];
    new_buttons |= ((report360->buttons & 64) == 64)
    << GetOwner(this)->mapping[6];
    new_buttons |= ((report360->buttons & 128) == 128)
    << GetOwner(this)->mapping[7];
    new_buttons |= ((report360->buttons & 256) == 256)
    << GetOwner(this)->mapping[8];
    new_buttons |= ((report360->buttons & 512) == 512)
    << GetOwner(this)->mapping[9];
    new_buttons |= ((report360->buttons & 1024) == 1024)
    << GetOwner(this)->mapping[10];
    new_buttons |= ((report360->buttons & 4096) == 4096)
    << GetOwner(this)->mapping[11];
    new_buttons |= ((report360->buttons & 8192) == 8192)
    << GetOwner(this)->mapping[12];
    new_buttons |= ((report360->buttons & 16384) == 16384)
    << GetOwner(this)->mapping[13];
    new_buttons |= ((report360->buttons & 32768) == 32768)
    << GetOwner(this)->mapping[14];

    //    IOLog("BUTTON PACKET - %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,
    //    %d, %d, %d\n", GetOwner(this)->mapping[0], GetOwner(this)->mapping[1],
    //    GetOwner(this)->mapping[2], GetOwner(this)->mapping[3],
    //    GetOwner(this)->mapping[4], GetOwner(this)->mapping[5],
    //    GetOwner(this)->mapping[6], GetOwner(this)->mapping[7],
    //    GetOwner(this)->mapping[8], GetOwner(this)->mapping[9],
    //    GetOwner(this)->mapping[10], GetOwner(this)->mapping[11],
    //    GetOwner(this)->mapping[12], GetOwner(this)->mapping[13],
    //    GetOwner(this)->mapping[14]);
...
#pragma mark - XboxOriginalControllerClass
  ...
  OSDefineMetaClassAndStructors(XboxOriginalControllerClass,
  ...
  // See
  // https://github.com/Grumbel/xboxdrv/blob/master/src/controller/xbox_controller.cpp
  ...
  IOReturn XboxOriginalControllerClass::handleReport(
                                                     IOMemoryDescriptor *descriptor, IOHIDReportType reportType,
                                                     IOOptionBits options) {
    //    IOLog("%s\n", __FUNCTION__);
    ...
    IOReturn ret =
    Xbox360ControllerClass::handleReport(descriptor, reportType, options);
    // IOLog("%s END\n", __FUNCTION__);
    return ret;
  }
...
#pragma mark - XboxOneControllerClass
  ...
  struct XBOXONE_IN_FIGHTSTICK_REPORT {
    XBOXONE_HEADER header;
    UInt16 buttons;
    UInt16 trigL, trigR;
    XBOX360_HAT left, right;
  ...
  struct XBOXONE_IN_WHEEL_REPORT {
    XBOXONE_HEADER header;
    UInt16 buttons;
    union {
  ...
  struct XBOXONE_ELITE_IN_REPORT {
    XBOXONE_HEADER header;
    UInt16 buttons;
    UInt16 trigL, trigR;
    XBOX360_HAT left, right;
  ...
  struct XBOXONE_IN_GUIDE_REPORT {
    XBOXONE_HEADER header;
    UInt8 state;
    UInt8 dummy;
  } PACKED;

  struct XBOXONE_OUT_GUIDE_REPORT {
    XBOXONE_HEADER header;
    UInt8 data[4];
    UInt8 zero[5];
  } PACKED;

  struct XBOXONE_OUT_RUMBLE {
    XBOXONE_HEADER header;
    ...
    UInt8 period;  // Period of time between pulses. DO NOT INCLUDE WHEN
                   // SUBSTRUCTURE IS 0x09
    UInt8 extra;
  } PACKED;

  struct XBOXONE_OUT_LED {
  ...
  void XboxOneControllerClass::convertFromXboxOne(void *buffer,
    ...
    if (packetSize == 0x1a)  // Fight Stick
      {
      if ((0x80 & reportXone->true_trigR) == 0x80) {
        trigL = 255;
      }
      if ((0x40 & reportXone->true_trigR) == 0x40) {
        trigR = 255;
      }
      ...
      right = reportXone->right;
        ...
        left.y =
        wheelReport->clutch *
        128;  // Clutch is 0-255. Upconvert to half signed 16 range. (0 - 32640)
        right = XBOX360_HAT{};  // TODO(bojanin): how was {} allowed before? (right = {};)
        } else        {
  ...
  IOReturn XboxOneControllerClass::handleReport(IOMemoryDescriptor *descriptor,
                                                IOHIDReportType reportType,
                                                IOOptionBits options) {
    if (descriptor->getLength() >= sizeof(XBOXONE_IN_GUIDE_REPORT)) {
      ...
      OSDynamicCast(IOBufferMemoryDescriptor, descriptor);
      if (desc != NULL) {
        XBOXONE_ELITE_IN_REPORT *report =
        (XBOXONE_ELITE_IN_REPORT *)desc->getBytesNoCopy();
        if ((report->header.command == 0x07) &&
            (report->header.size == (sizeof(XBOXONE_IN_GUIDE_REPORT) - 4))) {
          XBOXONE_IN_GUIDE_REPORT *guideReport =
          ...
          oldReport->buttons ^=
          (-isXboxOneGuideButtonPressed ^ oldReport->buttons) &
          (1 << GetOwner(this)->mapping[10]);
          ...
          if (!(GetOwner(this)->noMapping)) remapButtons(report360);
          GetOwner(this)->fiddleReport(report360->left, report360->right);

          if (GetOwner(this)->swapSticks) remapAxes(report360);
  ...
  IOReturn XboxOneControllerClass::setReport(IOMemoryDescriptor *report,
                                             IOHIDReportType reportType,
                                             IOOptionBits options) {
    ...
    //        ((unsigned char*)data)[1], ((unsigned char*)data)[2], ((unsigned
    //        char*)data)[3]);
    UInt8 rumbleType;
    switch (data[0])  //(header.command)
      {
        ...
        //            data[1], data[2], data[3], rumble.reserved2);
