...
constexpr const char *kDriverSettingKey = "DeviceData";
constexpr const char *kIOSerialDeviceType = "Serial360Device";
...
OSDefineMetaClassAndStructors(Xbox360Peripheral, IOService)
 ...
 public:
  ...
  ~LockRequired() { IOLockUnlock(_lock); }
};
...
void Xbox360Peripheral::SendSpecial(UInt16 value) {
  ...
  controlReq.bmRequestType =
      USBmakebmRequestType(kUSBOut, kUSBVendor, kUSBInterface);
...
void Xbox360Peripheral::SendInit(UInt16 value, UInt16 index) {
  ...
  controlReq.wLength = 0;
  controlReq.pData = NULL;
  device->DeviceRequest(&controlReq, 100, 100,
                        NULL);  // Will fail - but device should still act on it
}
...
bool Xbox360Peripheral::SendSwitch(bool sendOut) {
  ...
  controlReq.bmRequestType =
      USBmakebmRequestType(sendOut ? kUSBOut : kUSBIn, kUSBVendor, kUSBDevice);
...
void Xbox360Peripheral::ChatPadTimerActionWrapper(OSObject *owner,
...
void Xbox360Peripheral::ChatPadTimerAction(IOTimerEventSource *sender) {
  ...
  sender->setTimeoutMS(
      nextTime);  // Todo: Make it take into account function execution time?
  serialHeard = false;
...
void Xbox360Peripheral::readSettings(void) {
  ...
  OSDictionary *dataDictionary =
      OSDynamicCast(OSDictionary, getProperty(kDriverSettingKey));
...
bool Xbox360Peripheral::init(OSDictionary *propTable) {
  bool res = IOService::init(propTable);
...
bool Xbox360Peripheral::start(IOService *provider) {
  ...
  // NOTE(bojanin): This method doesnt exist anymore, just gonna use next.
  ...
  if (device->SetConfiguration(cd->bConfigurationValue, true) !=
      kIOReturnSuccess) {
  ...
  // Get release
  {
    const UInt16 release = device->GetDeviceRelease();
    switch (release) {
      default:
...
interfacefound:
  ...
  outPipe->retain();
  // Get a buffer
  inBuffer = IOBufferMemoryDescriptor::inTaskWithOptions(
      kernel_task, kIODirectionIn, GetMaxPacketSize(inPipe));
  if (inBuffer == NULL) {
  ...
  serialInBuffer = IOBufferMemoryDescriptor::inTaskWithOptions(
      kernel_task, kIODirectionIn, GetMaxPacketSize(serialInPipe));
  ...
  serialTimer =
      IOTimerEventSource::timerEventSource(this, ChatPadTimerActionWrapper);
  if (serialTimer == NULL) {
  ...
  if ((workloop == NULL) ||
      (workloop->addEventSource(serialTimer) != kIOReturnSuccess)) {
  ...
  // Begin reading
  if (!QueueSerialRead()) goto fail;
nochat:
  if (!QueueRead()) goto fail;
  if (controllerType == XboxOne || controllerType == XboxOnePretend360) {
    UInt8 xoneInit0[] = {0x01, 0x20, 0x00, 0x09, 0x00, 0x04, 0x20,
                         0x3a, 0x00, 0x00, 0x00, 0x80, 0x00};
    UInt8 xoneInit1[] = {0x05, 0x20, 0x00, 0x01, 0x00};
    UInt8 xoneInit2[] = {0x09, 0x00, 0x00, 0x09, 0x00, 0x0F, 0x00,
                         0x00, 0x1D, 0x1D, 0xFF, 0x00, 0x00};
    UInt8 xoneInit3[] = {0x09, 0x00, 0x00, 0x09, 0x00, 0x0F, 0x00,
...
bool Xbox360Peripheral::QueueRead(void) {
  ...
  if ((inPipe == NULL) || (inBuffer == NULL)) return false;
...
bool Xbox360Peripheral::QueueSerialRead(void) {
  ...
  if ((serialInPipe == NULL) || (serialInBuffer == NULL)) return false;
  complete.target = this;
  complete.action = SerialReadCompleteInternal;
  complete.parameter = serialInBuffer;
  err = serialInPipe->Read(serialInBuffer, 0, 0, serialInBuffer->getLength(),
                           &complete);
  if (err == kIOReturnSuccess) {
    return true;
  } else {
...
bool Xbox360Peripheral::QueueWrite(const void *bytes, UInt32 length) {
  ...
  outBuffer = IOBufferMemoryDescriptor::inTaskWithOptions(
...
bool Xbox360Peripheral::willTerminate(IOService *provider,
                                      IOOptionBits options) {
  ReleaseAll();

  return IOSurface::willTerminate(provider, options);
}
...
// Handle termination
bool Xbox360Peripheral::didTerminate(IOService *provider, IOOptionBits options,
                                     bool *defer) {
  // release all objects used and close the device
  ReleaseAll();
  return IOSurface::didTerminate(provider, options, defer);
}

// Handle message sent to the driver
IOReturn Xbox360Peripheral::message(UInt32 type, IOService *provider,
                                    void *argument) {
  switch (type) {
    case kIOMessageServiceIsTerminated:
    case kIOMessageServiceIsRequestingClose:
    default:
      return IOSurface::message(type, provider, argument);
  }
}
  ...
  if (deadzoneLeft != 0) {
    if (relativeLeft) {
      if ((getAbsolute(left.x) < deadzoneLeft) &&
          (getAbsolute(left.y) < deadzoneLeft)) {
        left.x = 0;
        left.y = 0;
      } else if (deadOffLeft) {
  ...
  if (deadzoneRight != 0) {
    if (relativeRight) {
      if ((getAbsolute(right.x) < deadzoneRight) &&
          (getAbsolute(right.y) < deadzoneRight)) {
        right.x = 0;
        right.y = 0;
...
void Xbox360Peripheral::ReadCompleteInternal(void *target, void *parameter,
                                             IOReturn status,
                                             UInt32 bufferSizeRemaining) {
  if (target != NULL)
    ((Xbox360Peripheral *)target)
        ->ReadComplete(parameter, status, bufferSizeRemaining);
}

void Xbox360Peripheral::SerialReadCompleteInternal(void *target,
                                                   void *parameter,
                                                   IOReturn status,
                                                   UInt32 bufferSizeRemaining) {
  if (target != NULL)
    ((Xbox360Peripheral *)target)
        ->SerialReadComplete(parameter, status, bufferSizeRemaining);
}

// This forwards a completed write notification to a member function
void Xbox360Peripheral::WriteCompleteInternal(void *target, void *parameter,
                                              IOReturn status,
                                              UInt32 bufferSizeRemaining) {
  if (target != NULL)
    ((Xbox360Peripheral *)target)
        ->WriteComplete(parameter, status, bufferSizeRemaining);
}

// This handles a completed asynchronous read
void Xbox360Peripheral::ReadComplete(void *parameter, IOReturn status,
                                     UInt32 bufferSizeRemaining) {
  if (padHandler != NULL)  // avoid deadlock with release
  {
    ...
    switch (status) {
      case kIOReturnOverrun:
        ...
        if (inPipe != NULL) inPipe->ClearStall();
        // Fall through
      case kIOReturnSuccess:
        if (inBuffer != NULL) {
          const XBOX360_IN_REPORT *report =
              (const XBOX360_IN_REPORT *)inBuffer->getBytesNoCopy();
          if (((report->header.command == inReport) &&
               (report->header.size == sizeof(XBOX360_IN_REPORT))) ||
              (report->header.command == 0x20) ||
              (report->header.command == 0x07)) /* Xbox One */ {
...
void Xbox360Peripheral::SerialReadComplete(void *parameter, IOReturn status,
                                           UInt32 bufferSizeRemaining) {
  if (padHandler != NULL)  // avoid deadlock with release
  {
    LockRequired locker(mainLock);
    ...
    switch (status) {
      case kIOReturnOverrun:
        ...
        if (serialInPipe != NULL) serialInPipe->ClearStall();
      // Fall through
      case kIOReturnSuccess:
        serialHeard = true;
        if (serialInBuffer != NULL)
          SerialMessage(serialInBuffer,
                        serialInBuffer->getCapacity() - bufferSizeRemaining);
        break;
    ...
    if (reread) QueueSerialRead();
  }
}

// Handle a completed asynchronous write
void Xbox360Peripheral::WriteComplete(void *parameter, IOReturn status,
                                      UInt32 bufferSizeRemaining) {
...
IOReturn Xbox360Peripheral::setProperties(OSObject *properties) {
  ...
  if (dictionary != NULL) {
    dictionary->setObject(OSString::withCString("ControllerType"),
                          OSNumber::withNumber(controllerType, 8));
...
void Xbox360Peripheral::PadConnect(void) {
  ...
  if (padHandler != NULL) {
    ...
    OSDictionary *dictionary = OSDictionary::withObjects(
        objects, keys, sizeof(keys) / sizeof(keys[0]));
...
void Xbox360Peripheral::SerialConnect(void) {
  ...
  if (serialHandler != NULL) {
    ...
    OSDictionary *dictionary = OSDictionary::withObjects(
        objects, keys, sizeof(keys) / sizeof(keys[0]), 0);
...
void Xbox360Peripheral::SerialMessage(IOBufferMemoryDescriptor *data,
                                      size_t length) {
  if (serialHandler != NULL) {
