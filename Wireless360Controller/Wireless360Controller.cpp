/*
    ...
    Wireless360Controller.cpp - main source of the standard wireless controller
   driver
  ...
  if (deadzoneLeft != 0) {
    if (relativeLeft) {
      if ((getAbsolute(report->left.x) < deadzoneLeft) &&
          (getAbsolute(report->left.y) < deadzoneLeft)) {
        report->left.x = 0;
        report->left.y = 0;
      } else if (deadOffLeft) {
  ...
  if (deadzoneRight != 0) {
    if (relativeRight) {
      if ((getAbsolute(report->right.x) < deadzoneRight) &&
          (getAbsolute(report->right.y) < deadzoneRight)) {
        report->right.x = 0;
        report->right.y = 0;
...
void Wireless360Controller::remapButtons(void *buffer) {
  ...
  //    mapping[4], mapping[5], mapping[6], mapping[7], mapping[8], mapping[9],
...
void Wireless360Controller::receivedHIDupdate(unsigned char *data, int length) {
  ...
  if (!noMapping) remapButtons(data);
  if (swapSticks) remapAxes(data);
  super::receivedHIDupdate(data, length);
}

void Wireless360Controller::SetRumbleMotors(unsigned char large,
                                            unsigned char small) {
  unsigned char buf[] = {0x00,  0x01, 0x0f, 0xc0, 0x00, large,
                         small, 0x00, 0x00, 0x00, 0x00, 0x00};
  WirelessDevice *device = OSDynamicCast(WirelessDevice, getProvider());

  if (device != NULL) device->SendPacket(buf, sizeof(buf));
}

IOReturn Wireless360Controller::setReport(IOMemoryDescriptor *report,
...
IOReturn Wireless360Controller::newReportDescriptor(
    IOMemoryDescriptor **descriptor) const {
  IOBufferMemoryDescriptor *buffer =
      IOBufferMemoryDescriptor::inTaskWithOptions(kernel_task, kIODirectionOut,
