...
#define super IOHIDDevice
    ...
    const char weirdStart[] = {0x00, 0x00, 0x00, 0x40, 0x00, 0x00,
                               0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

void WirelessHIDDevice::ChatPadTimerActionWrapper(OSObject *owner,
                                                  IOTimerEventSource *sender) {
  WirelessHIDDevice *device = OSDynamicCast(WirelessHIDDevice, owner);

  // Automatic shutoff
  device->serialTimerCount++;
  if (device->serialTimerCount > POWEROFF_TIMEOUT) device->PowerOff();
  // Reset
...
void WirelessHIDDevice::SetLEDs(int mode) {
  unsigned char buf[] = {
      0x00, 0x00, 0x08, (unsigned char)(0x40 + (mode % 0x0e)),
      0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00};
...
unsigned char WirelessHIDDevice::GetBatteryLevel(void) { return battery; }

void WirelessHIDDevice::PowerOff(void) {
  static const unsigned char buf[] = {0x00, 0x00, 0x08, 0xC0, 0x00, 0x00,
                                      0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  WirelessDevice *device = OSDynamicCast(WirelessDevice, getProvider());
...
IOReturn WirelessHIDDevice::setReport(IOMemoryDescriptor *report,
                                      IOHIDReportType reportType,
                                      IOOptionBits options) {
  char data[2];

  if (report->readBytes(0, data, 2) < 2) return kIOReturnUnsupported;
...
bool WirelessHIDDevice::handleStart(IOService *provider) {
  ...
  if (!super::handleStart(provider)) goto fail;

  device = OSDynamicCast(WirelessDevice, provider);
  if (device == NULL) goto fail;
...
void WirelessHIDDevice::handleStop(IOService *provider) {
  ...
  if (device != NULL) device->RegisterWatcher(NULL, NULL, NULL);

  if (serialTimer != NULL) {
    serialTimer->cancelTimeout();
    IOWorkLoop *workloop = getWorkLoop();
    if (workloop != NULL) workloop->removeEventSource(serialTimer);
    serialTimer->release();
    serialTimer = NULL;
  }
...
void WirelessHIDDevice::receivedMessage(IOMemoryDescriptor *data) {
  ...
  if (data->getLength() != 29) return;

  data->readBytes(0, buf, 29);

  switch (buf[1]) {
    case 0x0f:  // Initial info
      if (buf[16] == 0x13) receivedUpdate(0x13, buf + 17);
    ...
    case 0x01:  // HID info update
      if (buf[3] == 0xf0) receivedHIDupdate(buf + 4, buf[5]);
      break;
...
void WirelessHIDDevice::receivedUpdate(unsigned char type,
                                       unsigned char *data) {
  switch (type) {
...
void WirelessHIDDevice::receivedHIDupdate(unsigned char *data, int length) {
  ...
  report->release();
  if (err != kIOReturnSuccess) IOLog("handleReport return: 0x%.8x\n", err);
}

// Wrapper for notification of receiving data
void WirelessHIDDevice::_receivedData(void *target, WirelessDevice *sender,
                                      void *parameter) {
