...
bool WirelessGamingReceiver::start(IOService *provider) {
  ...
  if (device->SetConfiguration(this, cd->bConfigurationValue, true) !=
      kIOReturnSuccess) {
  ...
  iConnection = 0;
  iOther = 0;
  while ((interface = device->FindNextInterface(interface,
    ...
    switch (interface->GetInterfaceProtocol()) {
      case 129:  // Controller
        ...
        connections[iConnection].controllerIn =
            interface->FindNextPipe(NULL, &pipeRequest);
        ...
        connections[iConnection].controllerOut =
            interface->FindNextPipe(NULL, &pipeRequest);
  ...
  if (iConnection != iOther) IOLog("start - interface mismatch?\n");
...
// Handle termination
bool WirelessGamingReceiver::didTerminate(IOService *provider,
...
IOReturn WirelessGamingReceiver::message(UInt32 type, IOService *provider,
                                         void *argument) {
  // IOLog("Message\n");
#if 0
  switch(type) {
...
bool WirelessGamingReceiver::QueueRead(int index) {
  ...
  data->buffer = IOBufferMemoryDescriptor::inTaskWithOptions(
      kernel_task, kIODirectionIn,
      GetMaxPacketSize(connections[index].controllerIn));
  if (data->buffer == NULL) {
  ...
  err = connections[index].controllerIn->Read(
...
void WirelessGamingReceiver::ReadComplete(void *parameter, IOReturn status,
...
bool WirelessGamingReceiver::QueueWrite(int index, const void *bytes,
                                        UInt32 length) {
  ...
  err = connections[index].controllerOut->Write(outBuffer, 0, 0, length,
                                                &complete);
...
void WirelessGamingReceiver::WriteComplete(void *parameter, IOReturn status,
                                           UInt32 bufferSizeRemaining) {
...
void WirelessGamingReceiver::_ReadComplete(void *target, void *parameter,
                                           IOReturn status,
                                           UInt32 bufferSizeRemaining) {
  if (target != NULL)
    ((WirelessGamingReceiver *)target)
        ->ReadComplete(parameter, status, bufferSizeRemaining);
}

// Static wrapper for write notifications
void WirelessGamingReceiver::_WriteComplete(void *target, void *parameter,
                                            IOReturn status,
                                            UInt32 bufferSizeRemaining) {
  if (target != NULL)
    ((WirelessGamingReceiver *)target)
...
void WirelessGamingReceiver::ProcessMessage(int index,
  ...
  if ((length == 2) && (data[0] == 0x08)) {
    if (data[1] == 0x00) {
      ...
      if (connections[index].service == NULL) {
        ...
        for (i = 0; !ready && (i < j); i++) {
          data = OSDynamicCast(IOMemoryDescriptor,
                               connections[index].inputArray->getObject(i));
          data->readBytes(1, &c, 1);
          if (c == 0x0f) ready = true;
        }
  ...
  if (connections[index].service == NULL) InstantiateService(index);
  if (connections[index].service != NULL) {
...
void WirelessGamingReceiver::InstantiateService(int index) {
  ...
  if (connections[index].service != NULL) {
    ...
    if (connections[index].service->init(dictionary)) {
      ...
      if (IsDataQueued(index)) connections[index].service->NewData();
