...
static IOCFPlugInInterface functionMapXBOBT_IOCFPlugInInterface = {
    ...
    NULL,
    // IUnknown
    &FeedbackXBOBT::sQueryInterface, &FeedbackXBOBT::sAddRef,
    &FeedbackXBOBT::sRelease,
    // IOCFPlugInInterface
    1, 0,  // Version
    &FeedbackXBOBT::sProbe, &FeedbackXBOBT::sStart, &FeedbackXBOBT::sStop};

static IOForceFeedbackDeviceInterface
    functionMapXBOBT_IOForceFeedbackDeviceInterface = {
        // Padding required for COM
        NULL,
        // IUnknown
        &FeedbackXBOBT::sQueryInterface, &FeedbackXBOBT::sAddRef,
        ...
        &FeedbackXBOBT::sGetVersion, &FeedbackXBOBT::sInitializeTerminate,
        &FeedbackXBOBT::sDestroyEffect, &FeedbackXBOBT::sDownloadEffect,
        &FeedbackXBOBT::sEscape, &FeedbackXBOBT::sGetEffectStatus,
        &FeedbackXBOBT::sGetForceFeedbackCapabilities,
        &FeedbackXBOBT::sGetForceFeedbackState,
        &FeedbackXBOBT::sSendForceFeedbackCommand, &FeedbackXBOBT::sSetProperty,
        &FeedbackXBOBT::sStartEffect, &FeedbackXBOBT::sStopEffect};
...
FeedbackXBOBT::FeedbackXBOBT()
  ...
  iIOCFPlugInInterface.pseudoVTable =
      (IUnknownVTbl *)&functionMapXBOBT_IOCFPlugInInterface;
  iIOCFPlugInInterface.obj = this;

  iIOForceFeedbackDeviceInterface.pseudoVTable =
      (IUnknownVTbl *)&functionMapXBOBT_IOForceFeedbackDeviceInterface;
...
HRESULT FeedbackXBOBT::QueryInterface(REFIID iid, LPVOID *ppv) {
  ...
  else if (CFEqual(interface, IUnknownUUID) ||
           CFEqual(interface, kIOCFPlugInInterfaceID))
...
ULONG FeedbackXBOBT::AddRef() { return ++fRefCount; }
...
IOReturn FeedbackXBOBT::Probe(CFDictionaryRef propertyTable,
...
IOReturn FeedbackXBOBT::Start(CFDictionaryRef propertyTable,
                              io_service_t service) {
  return FF_OK;
}

IOReturn FeedbackXBOBT::Stop(void) { return FF_OK; }
...
HRESULT FeedbackXBOBT::StartEffect(FFEffectDownloadID EffectHandle,
                                   FFEffectStartFlag Mode, UInt32 Count) {
  dispatch_sync(Queue, ^{
    for (FeedbackXBOEffectIterator effectIterator = EffectList.begin();
         effectIterator != EffectList.end(); ++effectIterator) {
...
HRESULT FeedbackXBOBT::DownloadEffect(CFUUIDRef EffectType,
                                      FFEffectDownloadID *EffectHandle,
                                      FFEFFECT *DiEffect,
  ...
  dispatch_sync(Queue, ^{
    ...
    if (Effect == NULL || Result == -1) {
      ...
      if (Flags & FFEP_TRIGGERREPEATINTERVAL) {
        Effect->DiEffect.dwTriggerRepeatInterval =
      ...
      if (Flags & FFEP_TYPESPECIFICPARAMS) {
        if (CFEqual(EffectType, kFFEffectType_CustomForce_ID)) {
          memcpy(&Effect->DiCustomForce, DiEffect->lpvTypeSpecificParams,
          ...
          memcpy(&Effect->DiConstantForce, DiEffect->lpvTypeSpecificParams,
                 DiEffect->cbTypeSpecificParams);
          Effect->DiEffect.lpvTypeSpecificParams = &Effect->DiConstantForce;
        } else if (CFEqual(EffectType, kFFEffectType_Square_ID) ||
                   CFEqual(EffectType, kFFEffectType_Sine_ID) ||
                   CFEqual(EffectType, kFFEffectType_Triangle_ID) ||
                   CFEqual(EffectType, kFFEffectType_SawtoothUp_ID) ||
                   CFEqual(EffectType, kFFEffectType_SawtoothDown_ID)) {
          memcpy(&Effect->DiPeriodic, DiEffect->lpvTypeSpecificParams,
                 DiEffect->cbTypeSpecificParams);
          Effect->DiEffect.lpvTypeSpecificParams = &Effect->DiPeriodic;
        } else if (CFEqual(EffectType, kFFEffectType_RampForce_ID)) {
          memcpy(&Effect->DiRampforce, DiEffect->lpvTypeSpecificParams,
                 DiEffect->cbTypeSpecificParams);
...
HRESULT FeedbackXBOBT::GetForceFeedbackState(
    ForceFeedbackDeviceState *DeviceState) {
...
HRESULT FeedbackXBOBT::GetForceFeedbackCapabilities(
  ...
  capabilities->supportedEffects =
      FFCAP_ET_CUSTOMFORCE | FFCAP_ET_CONSTANTFORCE | FFCAP_ET_RAMPFORCE |
...
HRESULT FeedbackXBOBT::InitializeTerminate(NumVersion APIversion,
  ...
  if (begin) {
    ...
    // From probe
    if ((hidDevice == 0) || !goodProbe(hidDevice)) {
...
HRESULT FeedbackXBOBT::Escape(FFEffectDownloadID downloadID,
                              FFEFFESCAPE *escape) {
...
void FeedbackXBOBT::SetForce(LONG LeftLevel, LONG RightLevel, LONG ltLevel,
                             LONG rtLevel) {
  // fprintf(stderr, "LS: %d; RS: %d\n", (unsigned char)MIN( 255, LeftLevel *
  // Gain / 10000 ), (unsigned char)MIN( 255, RightLevel * Gain / 10000 ));
  XboxOneBluetoothReport_t report = {0};
  report.reportID = 0x03;
  report.activationMask = 0x0f;
  report.ltMagnitude =
      (unsigned char)min(SCALE_MAX, ltLevel * (LONG)Gain / 10000);
  report.rtMagnitude =
      (unsigned char)min(SCALE_MAX, rtLevel * (LONG)Gain / 10000);
  report.leftMagnitude =
      (unsigned char)min(SCALE_MAX, LeftLevel * (LONG)Gain / 10000);
  report.rightMagnitude =
      (unsigned char)min(SCALE_MAX, RightLevel * (LONG)Gain / 10000);
  report.duration = 0x7f;
  report.loopCount = 10;

  if (!Manual) {
    IOReturn retVal = IOHIDDeviceSetReport(
        device, kIOHIDReportTypeOutput, report.reportID,
        (const uint8_t *)&report, sizeof(XboxOneBluetoothReport_t));
    if (retVal != 0) {
      printf(
          "IOHIDDeviceSetReport returned %d (system %d, subsystem %d, code "
          "%d)\n",
          retVal, err_get_system(retVal), err_get_sub(retVal),
          err_get_code(retVal));
    }
  }
}
...
void FeedbackXBOBT::EffectProc(void *params) {
  ...
  if (cThis->Actuator == true) {
    for (FeedbackXBOEffectIterator effectIterator = cThis->EffectList.begin();
         effectIterator != cThis->EffectList.end(); ++effectIterator) {
      if (((CurrentTimeUsingMach() - cThis->LastTime) * 1000 * 1000) >=
          effectIterator->DiEffect.dwSamplePeriod) {
        CalcResult =
            effectIterator->Calc(&LeftLevel, &RightLevel, &ltLevel, &rtLevel);
      }
    }
  }

  if ((cThis->PrvLeftLevel != LeftLevel ||
       cThis->PrvRightLevel != RightLevel) &&
      (CalcResult != -1)) {
    // fprintf(stderr, "PL: %d, PR: %d; L: %d, R: %d; \n", cThis->PrvLeftLevel,
    // cThis->PrvRightLevel, LeftLevel, RightLevel);
    cThis->SetForce((unsigned char)min(SCALE_MAX, LeftLevel * Gain / 10000),
                    (unsigned char)min(SCALE_MAX, RightLevel * Gain / 10000),
                    (unsigned char)min(SCALE_MAX, ltLevel * Gain / 10000),
                    (unsigned char)min(SCALE_MAX, rtLevel * Gain / 10000));
...
HRESULT FeedbackXBOBT::GetEffectStatus(FFEffectDownloadID EffectHandle,
...
IOReturn FeedbackXBOBT::sProbe(void *self, CFDictionaryRef propertyTable,
                               io_service_t service, SInt32 *order) {
  return getThis(self)->Probe(propertyTable, service, order);
}

IOReturn FeedbackXBOBT::sStart(void *self, CFDictionaryRef propertyTable,
                               io_service_t service) {
  return getThis(self)->Start(propertyTable, service);
}

IOReturn FeedbackXBOBT::sStop(void *self) { return getThis(self)->Stop(); }
...
HRESULT FeedbackXBOBT::sInitializeTerminate(void *self,
                                            NumVersion forceFeedbackAPIVersion,
                                            io_object_t hidDevice,
                                            boolean_t begin) {
  return FeedbackXBOBT::getThis(self)->InitializeTerminate(
      forceFeedbackAPIVersion, hidDevice, begin);
}

HRESULT FeedbackXBOBT::sDestroyEffect(void *self,
                                      FFEffectDownloadID downloadID) {
  return FeedbackXBOBT::getThis(self)->DestroyEffect(downloadID);
}

HRESULT FeedbackXBOBT::sDownloadEffect(void *self, CFUUIDRef effectType,
                                       FFEffectDownloadID *pDownloadID,
                                       FFEFFECT *pEffect,
                                       FFEffectParameterFlag flags) {
  return FeedbackXBOBT::getThis(self)->DownloadEffect(effectType, pDownloadID,
                                                      pEffect, flags);
}

HRESULT FeedbackXBOBT::sEscape(void *self, FFEffectDownloadID downloadID,
                               FFEFFESCAPE *pEscape) {
  return FeedbackXBOBT::getThis(self)->Escape(downloadID, pEscape);
}

HRESULT FeedbackXBOBT::sGetEffectStatus(void *self,
                                        FFEffectDownloadID downloadID,
                                        FFEffectStatusFlag *pStatusCode) {
  return FeedbackXBOBT::getThis(self)->GetEffectStatus(downloadID, pStatusCode);
}

HRESULT FeedbackXBOBT::sGetForceFeedbackState(
    void *self, ForceFeedbackDeviceState *pDeviceState) {
  return FeedbackXBOBT::getThis(self)->GetForceFeedbackState(pDeviceState);
}

HRESULT FeedbackXBOBT::sGetForceFeedbackCapabilities(
    void *self, FFCAPABILITIES *capabilities) {
  return FeedbackXBOBT::getThis(self)->GetForceFeedbackCapabilities(
      capabilities);
}

HRESULT FeedbackXBOBT::sSendForceFeedbackCommand(void *self,
                                                 FFCommandFlag state) {
  return FeedbackXBOBT::getThis(self)->SendForceFeedbackCommand(state);
}

HRESULT FeedbackXBOBT::sSetProperty(void *self, FFProperty property,
                                    void *pValue) {
  return FeedbackXBOBT::getThis(self)->SetProperty(property, pValue);
}

HRESULT FeedbackXBOBT::sStartEffect(void *self, FFEffectDownloadID downloadID,
                                    FFEffectStartFlag mode, UInt32 iterations) {
  return FeedbackXBOBT::getThis(self)->StartEffect(downloadID, mode,
                                                   iterations);
}
...
extern "C" void *FeedbackXBOBTFactory(CFAllocatorRef allocator,
                                      CFUUIDRef typeID) {
  void *result = NULL;
