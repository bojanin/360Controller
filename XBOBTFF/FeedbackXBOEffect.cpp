...
// CEffect
//----------------------------------------------------------------------------------------------
FeedbackXBOEffect::FeedbackXBOEffect()
    : Type(NULL),
      Status(0),
      PlayCount(0),
      StartTime(0),
...
FeedbackXBOEffect::FeedbackXBOEffect(FFEffectDownloadID theHand)
    : FeedbackXBOEffect() {
  Handle = theHand;
}

FeedbackXBOEffect::FeedbackXBOEffect(const FeedbackXBOEffect &src)
    : Type(src.Type),
      Handle(src.Handle),
      Status(src.Status),
...
// Calc
//----------------------------------------------------------------------------------------------
LONG FeedbackXBOEffect::Calc(LONG *LeftLevel, LONG *RightLevel, LONG *ltLevel,
                             LONG *rtLevel) {
  ...
  if (Status == FFEGES_PLAYING && BeginTime <= CurrentTime &&
      CurrentTime <= EndTime) {
    ...
    CalcEnvelope((ULONG)(Duration * 1000),
                 ...
                 &NormalRate, &AttackLevel, &FadeLevel);

    // CustomForce allows setting each channel separately
    if (CFEqual(Type, kFFEffectType_CustomForce_ID)) {
      if ((CurrentTimeUsingMach() - LastTime) * 1000 * 1000 <
          DiCustomForce.dwSamplePeriod) {
        return -1;
      } else {
        if (DiCustomForce.cChannels == 2) {
          WorkLeftLevel = ((DiCustomForce.rglForceData[2 * Index] * NormalRate +
                            AttackLevel + FadeLevel) /
                           100) *
                          DiEffect.dwGain / 10000;
          WorkRightLevel =
              ((DiCustomForce.rglForceData[2 * Index + 1] * NormalRate +
                AttackLevel + FadeLevel) /
               100) *
              DiEffect.dwGain / 10000;
          WorkLTLevel = 0;
          ...
          WorkLeftLevel = ((DiCustomForce.rglForceData[4 * Index] * NormalRate +
                            AttackLevel + FadeLevel) /
                           100) *
                          DiEffect.dwGain / 10000;
          WorkRightLevel =
              ((DiCustomForce.rglForceData[4 * Index + 1] * NormalRate +
                AttackLevel + FadeLevel) /
               100) *
              DiEffect.dwGain / 10000;
          WorkLTLevel =
              ((DiCustomForce.rglForceData[4 * Index + 2] * NormalRate +
                AttackLevel + FadeLevel) /
               100) *
              DiEffect.dwGain / 10000;
          WorkRTLevel =
              ((DiCustomForce.rglForceData[4 * Index + 3] * NormalRate +
                AttackLevel + FadeLevel) /
               100) *
              DiEffect.dwGain / 10000;
          // fprintf(stderr, "L:%d; R:%d\n", WorkLeftLevel, WorkRightLevel);
    ...
    // Regular commands treat controller as a single output (both channels are
    // together as one)
    else {
      CalcForce((ULONG)(Duration * 1000),
                (ULONG)(fmod(CurrentTime - BeginTime, Duration) * 1000),
                NormalRate, AttackLevel, FadeLevel, &NormalLevel);
      // fprintf(stderr, "DeltaT %f\n", CurrentTime - BeginTime);
      // fprintf(stderr, "Duration %f; NormalRate: %d; AttackLevel: %d;
...
// CalcEnvelope
//----------------------------------------------------------------------------------------------
void FeedbackXBOEffect::CalcEnvelope(ULONG Duration, ULONG CurrentPos,
                                     LONG *NormalRate, LONG *AttackLevel,
                                     LONG *FadeLevel) {
...
void FeedbackXBOEffect::CalcForce(ULONG Duration, ULONG CurrentPos,
                                  LONG NormalRate, LONG AttackLevel,
                                  LONG FadeLevel, LONG *NormalLevel) {
  LONG Magnitude = 0;
  LONG Period;
  LONG R;
  LONG Rate;
  ...
  if (CFEqual(Type, kFFEffectType_ConstantForce_ID)) {
    ...
    Magnitude =
        (DiRampforce.lStart * Rate + DiRampforce.lEnd * (100 - Rate)) / 100;
    Magnitude = (Magnitude * NormalRate + AttackLevel + FadeLevel) / 100;
  }
