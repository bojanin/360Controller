...
bool Device_Initialise(DeviceLink *link, io_object_t device) {
  ...
  ret = (*plugInInterface)
            ->QueryInterface(plugInInterface,
                             CFUUIDGetUUIDBytes(kIOHIDDeviceInterfaceID121),
                             (LPVOID *)(&link->interface));
...
bool Device_Send(DeviceLink *link, void *data, int length) {
  if (link->interface == NULL) {
    ...
    // char*)data)[0], ((unsigned char*)data)[1], ((unsigned char*)data)[2],
    // ((unsigned char*)data)[3]);
    IOReturn res = (*link->interface)
                       ->setReport(link->interface, kIOHIDReportTypeOutput, 0,
                                   data, length, 10000, NULL, NULL, NULL);
